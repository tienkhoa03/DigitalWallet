...
| Authorization | RBAC enforced at the service layer (not only in the controller). Two roles in MVP: `USER`, `ADMIN`, stored on `user.role`. `FRAUD_ANALYST` and multi-role-per-user are deferred — see ADR #9. | [../../project-info.md §2.2](../../project-info.md#22-roles-in-the-system), [../decisions/0009-rbac-roles.md](../decisions/0009-rbac-roles.md) |
...
- **Authorization model:** role-based. Two roles in MVP — `USER`, `ADMIN` — stored on `user.role` ([../decisions/0009-rbac-roles.md](../decisions/0009-rbac-roles.md)). RBAC is enforced at the service layer to align with SOC 2. `FRAUD_ANALYST` and multi-role-per-user are deferred and will return via ADR when manual unsuspend / analyst workflows ship.
