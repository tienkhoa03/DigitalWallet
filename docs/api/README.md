...
- **Documentation:** OpenAPI generated by the Quarkus SmallRye OpenAPI extension; hand-maintained at the inbound web adapter ([../../project-info.md §13](../../project-info.md#13-coding-conventions-highest-level-project-wide)).
- **Auth:** all endpoints except sign-up and login require a stateless JWT (`Authorization: Bearer <token>`), signed with ES256. RBAC is enforced at the application service (use case), not only in the inbound web adapter ([../../project-info.md §8](../../project-info.md#8-security-baseline)).
...
| Method | Path | Purpose | Auth | Request body | Response body | Error keys |
|---|---|---|---|---|---|---|
| `POST` | `/budgets` | Create a monthly budget plan with per-category buckets (FR4.1). The budget inherits the caller's immutable `base_currency` — buckets do not carry a per-row currency. `month` is `YYYY-MM-01`. | `USER` | `{ month, buckets: [ { category_id, planned_amount, threshold_percent? } ] }` | `{ budget_id, month, buckets }` | `budget.duplicate_month`, `validation.invalid_payload` |
