...
- **RBAC at the service layer.** Authorization is enforced inside the service layer, not only by JAX-RS annotations on the controller. Two roles in MVP: `USER`, `ADMIN`, stored on `account.role`. `FRAUD_ANALYST` and multi-role-per-account are deferred — see ADR #9.
