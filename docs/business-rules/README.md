...
- **Audit log.** Authentication events, role grants, transfers, fraud-rule changes, fraud-driven blocks (FR2.1–FR2.2), account suspensions / un-suspensions (FR2.4), and admin reads of user data are recorded in the immutable `audit_log` append-only table. Required for SOC 2 alignment.
