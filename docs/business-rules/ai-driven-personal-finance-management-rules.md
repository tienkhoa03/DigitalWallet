...
- **Rule:** A user holds at most one budget per calendar month per account. Each budget owns a set of buckets, one per category, each with a `planned_amount` and an optional `threshold_percent` (see FR4.3).
...
- **Enforced in:** `pfm/service/` budget service; DB `UNIQUE(account_id, month)` on `budget` and `UNIQUE(budget_id, category_id)` on `bucket`. `(verify)`
...
- **Rule:** The PFM Kafka consumer drains `transaction-events` and updates the spent amount on the matching bucket — keyed by `(account_id, month_of(transaction_timestamp), category_id)`. Updates go to the **Redis hot read-model**; the Postgres materialized view is refreshed on a schedule as the durable backup and rebuild source. Direct `UPDATE`s against ledger tables are forbidden (NFR6).
...
- **Failure mode:**
  ...
  - Late event arriving after a month boundary → routed to the previous month's bucket based on `transaction_timestamp`, not wall-clock (NFR7).
...
- **Rule (event time):** Every PFM aggregation uses `transaction_timestamp` from the Kafka payload, not the consumer's wall-clock or DB-insert time.
