...
- **Rule:** Both `WS /admin/ws/alerts` and `GET /admin/metrics/live` enforce the `ADMIN` role at the service layer; the WebSocket handshake validates the JWT before accepting the upgrade.
