...
erDiagram
    ...
    ACCOUNT ||--o{ IDEMPOTENCY_RECORD : "issues"
...
|---|---|---|
| `ROLE_ASSIGNMENT` table (multi-role per account) | Single-role-per-account is simpler. The `role` column on `account` covers `USER` / `ADMIN` for MVP. | When an account must hold more than one role simultaneously, or when SOC 2 access-review history needs a separate audit table. |
...
- `ACCOUNT.base_currency` is **immutable** and chosen at signup. Every budget is implicitly scoped to it (no `BUDGET.currency` column).
...
Identity record per account. Owns wallets, budgets, and fraud alerts. The MVP holds the role inline on this row (no separate `role_assignment` table — see MVP scope notes).
...
| `role` | `varchar` | NOT NULL CHECK (in `USER`,`ADMIN`) DEFAULT `'USER'` | Single RBAC role per account; see [../decisions/0009-rbac-roles.md](../decisions/0009-rbac-roles.md). |
| `base_currency` | `char(3)` | NOT NULL | Chosen at signup, **immutable**. Every budget owned by this account is implicitly scoped to this currency; the PFM consumer converts cross-currency spending via the snapshotted FX rate on `transaction`. |
...
> Note: `account` is a plain identifier in PostgreSQL — no quoting needed. It replaces the original `user` table name, which collided with the SQL reserved word and had to be double-quoted (`"user"`); the rename removes that friction.

### `wallet`

A balance-bearing record scoped to a single currency ([../../project-info.md §9](../../project-info.md#9-domain-glossary)). An account MAY own multiple wallets, including more than one in the same currency — siblings are disambiguated by a user-supplied `label` ([../decisions/0006-multi-currency-model.md](../decisions/0006-multi-currency-model.md)).
...
| UNIQUE | `(account_id, label)` | — | Labels are unique per account so a wallet can be picked unambiguously in the UI. **No** `UNIQUE (account_id, currency)` — multiple wallets in the same currency are a first-class case (ADR #6). |
...
|---|---|---|---|
| `id` | `uuid` | PK | — |
| `account_id` | `uuid` | FK → `account.id`, NOT NULL | Subject account. Always present (including alerts raised from a block-path attempt with no ledger row). |
...
| `account_id` | `uuid` | FK → `account.id`, NOT NULL | Scope keys per account — prevents cross-tenant key collision. |
...
| UNIQUE | `(account_id, endpoint, idempotency_key)` | — | The constraint that makes `INSERT … ON CONFLICT DO NOTHING` work as the claim primitive. |

### `budget`

Monthly per-account spending plan (FR4.1). Budget currency is implicit — every budget is scoped to the owning account's `base_currency`.
...
| UNIQUE | `(account_id, month)` | — | One plan per month per account. |
...
| `planned_amount` | `numeric(19,4)` | NOT NULL CHECK (`planned_amount > 0`) | Cap chosen by the user, in the owning account's `base_currency`. |
...
| `aggregate_type` | `varchar` | NOT NULL | e.g. `TRANSACTION`, `ACCOUNT`. |
...
Backs NFR6 — durable backup and rebuild source for the Redis hot read-model. Computed from `transaction` joined to `budget_bucket` by `(account_id, category_id, month_of(event_timestamp))`, refreshed by a scheduled job. See [../decisions/0004-cqrs-budget-read-model.md](../decisions/0004-cqrs-budget-read-model.md).
...
- **Foreign keys:** `<referenced_table_singular>_id` (e.g. `account_id`, `wallet_id`, `category_id`).
