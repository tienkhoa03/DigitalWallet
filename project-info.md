...
| Fraud analyst | Investigate alerts surfaced by the engine, tune thresholds | occasional |

### 2.2 Roles in the system

| Role | Permissions | Notes |
|---|---|---|
| `USER` | own wallet CRUD, transfer, statement, budgets, PFM advice | default role on signup |
| `ADMIN` | live metrics dashboard, system-wide read, user/role management | needed for FR3.1 |
| `FRAUD_ANALYST` | fraud alert stream, manual flag/unflag, fraud-rule threshold tuning | needed for FR3.2 — kept separate from ADMIN to enforce least-privilege |
...
|---|---|---|
| PostgreSQL 16 | Authoritative ledger (accounts, wallets, transactions, outbox, budgets) | ACID, `numeric(19,4)` for money, `timestamptz` for timestamps; materialized views allowed for PFM read model (NFR6) |
...
- **Authorization model:** Role-based with three roles: `USER`, `ADMIN`, `FRAUD_ANALYST` (see §2.2).
- **PII handled:** email, full name, account number, wallet balance, transaction history, category labels. No card / bank credentials are stored (deposits are simulated).
- **Compliance constraints:** **SOC 2** oriented — implies (a) immutable audit log for all money mutations and admin actions, (b) RBAC enforced at the service layer (not only in the controller), (c) change-management traceability via Conventional Commits + PR review, (d) access reviews for the `ADMIN` and `FRAUD_ANALYST` roles.
...
| 12 | Compliance constraints? | ✅ Answered | SOC 2 orientation (audit log, RBAC at service layer, access review) |
