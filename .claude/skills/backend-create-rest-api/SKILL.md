...
4. **Constraints** — `Idempotency-Key required` (mutating money endpoints per [backend_coding.md §2](../../rules/backend_coding.md) and NFR3), `Pessimistic lock + Redis lock` (wallet mutations per [backend_coding.md §3](../../rules/backend_coding.md) and NFR1), or `Neither`.
...
5. **Service** at `<module>/service/` — `@Transactional` on the method; constructor injection only; service-layer RBAC re-check; for wallet mutations follow the Redis-lock → DB `PESSIMISTIC_WRITE` → outbox-write sequence. Per [backend_coding.md §3](../../rules/backend_coding.md) and [security.md §3](../../rules/security.md).
...
- [ ] Wallet mutations follow the Redis-lock → DB-row-lock → outbox-write sequence — [backend_coding.md §3](../../rules/backend_coding.md), NFR1.
