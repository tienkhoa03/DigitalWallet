...
- **Validation MUST be backend-aligned.** The Zod schema MUST mirror the server-side rule referenced below for each form:
  ...
  | Transfer | recipient identity | `to_account_id` only (no `account_number` in MVP); not equal to the sender wallet's owning account | [../../docs/business-rules/core-wallet-management-rules.md](../../docs/business-rules/core-wallet-management-rules.md) FR1.3 |
  ...
  | Create budget | `month` | ISO `YYYY-MM-01` (`date`, day = 1); unique per account. No per-bucket `currency_code` — the budget inherits the user's `base_currency`. | [../../docs/business-rules/ai-driven-personal-finance-management-rules.md](../../docs/business-rules/ai-driven-personal-finance-management-rules.md) FR4.1 |
...
- **Wallet selection & labelling:** the wallet-opening form lists every supported currency unconditionally and requires the user to pick a `label` ([../../docs/business-rules/core-wallet-management-rules.md](../../docs/business-rules/core-wallet-management-rules.md) FR1.1). An account MAY own multiple wallets in the same currency, so any wallet-picker UI (deposit, withdraw, transfer source) MUST render both `label` and `currency_code` together — Never identify a wallet by currency alone.
- **Real-time stream subscription:** the app MUST use a **single** shared WebSocket connection per session. Per-feature channels (`/admin/ws/alerts`, `/admin/ws/metrics`, `/accounts/ws/notifications`, advisor reply) are multiplexed through the shared client — Never open multiple raw `new WebSocket(...)` instances.
